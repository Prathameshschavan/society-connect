...
const UpdateUnitModal: React.FC<UpdateUnitModalProps> = ({
  ...
  // Populate form with unit data when unit changes
  useEffect(() => {
    if (unit) {
      setValue("unitNumber", unit.unit_number);
      setValue("squareFootage", unit.square_footage.toString());
      setValue("unitType", unit.unit_type as any);
      setValue("profileId", unit.profile_id || "");
    }
  }, [unit, setValue]);
